...
Most security suites are either too complex (built only for enterprise networks or expert laptops) or too restricted (like basic mobile VPNs). Ghost Mode is different ΓÇö it is a cross-platform, user-space defensive shield that secures both your mobile devices (via Termux on Android) and your computer workstations (via native Python engines on Windows, Linux, and macOS). It is built for anyone who wants sovereign protection across all their hardware, without needing complex setups or administrative root access.
...
## ≡ƒôè Global OS Compatibility Matrix

| Feature / Module | Android (Termux) | Windows PC | Linux (Ubuntu/Debian) | macOS |
| :--- | :---: | :---: | :---: | :---: |
| **≡ƒÄÖ∩╕Å Privacy Sentry (Mic/Cam Monitor)** | Γ£à Yes (Logcat Audit) | Γ£à Yes (Win32 API) | Γ£à Yes (lsof/procfs) | Γ£à Yes (AVFoundation) |
| **≡ƒ¢í∩╕Å Deception Sentry (SSH Honeypot)** | Γ£à Yes (Port 2222) | Γ£à Yes (Port 2222) | Γ£à Yes (Port 2222) | Γ£à Yes (Port 2222) |
| **≡ƒîÉ Sovereign Tor Routing** | Γ£à Yes (Tor daemon) | Γ£à Yes (Tor service) | Γ£à Yes (Tor daemon) | Γ£à Yes (Tor daemon) |
| **≡ƒÆ¼ Scam Sentry (SMS/Clipboard)** | Γ£à Yes (SMS / Copy) | Γ£à Yes (Clipboard scan) | Γ£à Yes (Clipboard scan) | Γ£à Yes (Clipboard scan) |
| **≡ƒöä Failover Daemon** | Γ£à Yes | Γ£à Yes | Γ£à Yes | Γ£à Yes |
| **≡ƒûÑ∩╕Å Web Dashboard Console** | Γ£à Yes (Port 8080) | Γ£à Yes (Port 8080) | Γ£à Yes (Port 8080) | Γ£à Yes (Port 8080) |
| **ΓÅ░ Automated Sentry Alerts** | Γ£à Yes (Telegram Sentry) | Γ£à Yes (Telegram Sentry) | Γ£à Yes (Telegram Sentry) | Γ£à Yes (Telegram Sentry) |
...
## ≡ƒîì Global OS Compatibility Matrix
...
| :--- | :--- | :--- | :--- |
| **Android (6.0+)** | ≡ƒƒó **Full (Recommended)** | Runs via **Termux** environment (No root required) | On-the-go mobile defense |
| **GrapheneOS / Pixel** | ≡ƒƒó **Full (Recommended)** | Runs via **Termux** (Supports sandboxed user profiles) | Hardened stealth operational device |
| **Windows 10 / 11** | ≡ƒƒó **Full** | Native Python engine (`ghost_mode_pc.py`) | Desktop workstation monitoring |
| **Windows WSL** | ≡ƒƒó **Full** | Runs in Windows Subsystem for Linux (WSL Ubuntu/Debian) | Developer environments |
| **Linux (Ubuntu/Debian)** | ≡ƒƒó **Full** | Native terminal daemon | General Linux security & servers |
| **Parrot Security OS / Kali**| ≡ƒƒó **Full** | Native terminal (Pre-installed security dependencies) | Advanced security environments |
| **macOS (Intel & M-Series)** | ≡ƒƒó **Full** | Native terminal via Homebrew | Apple developer workstations |
| **Raspberry Pi OS** | ≡ƒƒó **Full** | Native lightweight background daemon | Physical hardware security node |
| **iPhone / iOS** | ≡ƒö┤ **Unsupported** | Blocked by iOS App Sandbox restrictions | None (Unless jailbroken) |

---

## ΓÜí Installation & Quick Start

### ≡ƒñû Option 1: Android, Google Pixel & GrapheneOS
...
### ≡ƒÉº Option 3: Linux (Ubuntu, Debian, Arch)
...
### ≡ƒª£ Option 4: Parrot Security OS & Kali Linux
...
### ≡ƒìÄ Option 7: iPhone / iOS (Why it is Unsupported)
...
### ≡ƒûÑ∩╕Å On Desktop (Windows, macOS, Linux, Parrot, Raspberry Pi)
...
## ≡ƒº╣ Step-by-Step Clean Reset & Reinstallation Guide
...
### ≡ƒô▒ Path A: For Mobile Devices (Android / Termux)

#### ≡ƒ¢æ Phase 1: Wiping & Deleting Clean
...
3. **Hard Reset (Optional):**
   If packages are totally broken, go to Android Settings Γ₧ö Apps Γ₧ö Termux Γ₧ö Storage Γ₧ö **Clear Storage / Data** to factory reset Termux.

#### ≡ƒôÑ Phase 2: Fresh Installation & Launch
...
### ≡ƒûÑ∩╕Å Path B: For PC & Desktop Systems (Windows, Linux, macOS)
...
## ≡ƒöä Reboot & Power-Off Recovery Guide
...
### ≡ƒô▒ On Android (Termux)
1. Reopen the **Termux** app on your phone.
2. Launch the launcher menu:
   ```bash
   bash ~/ghost.sh
   ```
3. Run Option `[5] ≡ƒûÑ∩╕Å Open Dashboard` to start the web console daemon, then select Option `[1] ≡ƒÆÇ Run Security Scan` to restart background threat scanning.
4. **≡ƒÆí Automation Tip:** You can install the **Termux:Boot** addon app from F-Droid and add `bash ~/ghost.sh --boot` to your `~/.termux/boot/` script to start Aether automatically whenever your phone boots up!

### ≡ƒûÑ∩╕Å On Desktop PC (Windows, Linux, macOS)
...
3. Launch the engine:
   ...
   python ghost_mode_pc.py
   ```
4. **≡ƒÆí Automation Tip:** On Windows, you can add a shortcut of `ghost_mode_pc.py` to your Startup folder (`shell:startup`) to launch Aether automatically on login. On Linux, you can configure a systemd service file.

---

## ≡ƒÆ¼ Automated Scam Scanning Add-on
...
### ≡ƒô▒ Android Background Scan (Requires Termux:API)
...
### ≡ƒûÑ∩╕Å PC Background Scan (Requires Clipboard Access)
...
## ≡ƒîì Similar Tools (and why Ghost Mode is different)
...
| **Ghost Mode** | Γ¥î **No** | Γ£à **One command** | Γ£à **Defensive** |
...
#### ≡ƒöæ Token Recovery & Reset Guide
...
#### ≡ƒ¢á∩╕Å Sentry Bot Troubleshooting & Connection Diagnostics
...
*   **ΓÜá∩╕Å Crucial Concept:** Your **Chat ID is your personal Telegram User ID**. It is **not** unique to a specific bot.
...
*   **≡ƒô▒ On Mobile (Android / Termux):**
    ...
    2.  **Inspect connection error logs:**
        Run `cat ~/ghost_tools/ghost.log` or choose Option `[6] ≡ƒôï View Logs` in the launcher menu. Check for lines stating `ΓÜá∩╕Å Telegram alert failed: <reason>`. A timeout indicates Tor routing blocks or no internet connection.
    3.  **Ensure the daemon server is running:**
        The bot poller only works if Option `[5] ≡ƒûÑ∩╕Å Open Dashboard` is actively running. Restart the server daemon:
        ```bash
        pkill -f server_daemon.py
        bash ~/ghost.sh   # select option 5
        ```

*   **≡ƒûÑ∩╕Å On Desktop PC (Windows):**
...
*   **≡ƒûÑ∩╕Å On Desktop PC (Linux / macOS):**
...
*Every contribution ΓÇö no matter how small ΓÇö keeps Ghost Mode alive and free for everyone.*
